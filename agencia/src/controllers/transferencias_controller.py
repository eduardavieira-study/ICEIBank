...
def transferir(request: Request, body: TransferenciaRequest):
    ...
    registro.registrar("TRANSFERENCIA_DEBITO", ts_debito, {"idOrigem": id_origem, "idDestino": id_destino, "valor": valor})
    ...
    if agencia_destino == id_agencia:
        ...
        registro.registrar("TRANSFERENCIA_CREDITO", ts_credito, {"idOrigem": id_origem, "idDestino": id_destino, "valor": valor})
    ...
    agencia_info = next((a for a in config.AGENCIAS if a["id"] == agencia_destino), None)
    if not agencia_info:
    ...
    try:
        ...
        registro.registrar("TRANSFERENCIA_FALHOU", ts_falha, {
            "idOrigem": id_origem,
...
def creditar_remoto(request: Request, id: int, body: CreditoRemotoRequest):
    ...
    registro.registrar("TRANSFERENCIA_CREDITO_REMOTO", ts, {
        "idConta": id,
        "valor": body.valor,
        "origemAgencia": body.origemAgencia
    })
